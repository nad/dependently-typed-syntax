...
module Apply {T : Term-like Level.zero} (T↦Tm : T ↦ Tm) where
  ...
  mutual
    ...
    var x             /⊢ ρ = trans ⊙ (x /∋ ρ)
    ƛ σ′ t            /⊢ ρ = ƛ (σ′ /⊢t ρ) (t /⊢ ρ ↑)
    _·_ {τ = τ} t₁ t₂ /⊢ ρ = P.subst (λ v → _ ⊢ uc τ /̂ ⟦ ρ ⟧⇨ ↑̂ /̂ ŝub v)
                                     (P.sym $ t₂ /⊢-lemma ρ)
                                     ((t₁ /⊢ ρ) · (t₂ /⊢ ρ))
    ...
    abstract
      ...
      ƛ {σ = σ} {τ = τ} σ′ t /⊢-lemma ρ = begin
        c ⟦ t ⟧ /Val ⟦ ρ ⟧⇨      ≡⟨ P.refl ⟩
        c (⟦ t ⟧ /Val ⟦ ρ ↑ ⟧⇨)  ≡⟨ P.cong c (t /⊢-lemma (ρ ↑)) ⟩
        c ⟦ t /⊢ ρ ↑ ⟧           ∎
        where open P.≡-Reasoning

      _·_ {τ = τ} t₁ t₂ /⊢-lemma ρ = H.≅-to-≡ (begin
         ⟦ t₁ · t₂ ⟧ /Val ⟦ ρ ⟧⇨                        ≡⟨ P.refl ⟩
         (⟦ t₁ ⟧ /Val ⟦ ρ ⟧⇨) ˢ (⟦ t₂ ⟧ /Val ⟦ ρ ⟧⇨)    ≅⟨ ˢ-cong P.refl H.refl (H.refl {x = uc τ /̂ ⟦ ρ ⟧⇨ ↑̂})
                                                                  (H.≡-to-≅ $ t₁ /⊢-lemma ρ)
                                                                  (H.≡-to-≅ $ t₂ /⊢-lemma ρ) ⟩
         ⟦ t₁ /⊢ ρ ⟧ ˢ ⟦ t₂ /⊢ ρ ⟧                      ≡⟨ P.refl ⟩
         ⟦ (t₁ /⊢ ρ) · (t₂ /⊢ ρ) ⟧                      ≅⟨ ⟦⟧-cong P.refl
                                                                   (/̂-cong P.refl P.refl (H.refl {x = uc τ /̂ ⟦ ρ ⟧⇨ ↑̂})
                                                                           (ŝub-cong P.refl H.refl
                                                                                     (H.≡-to-≅ $ P.sym $ t₂ /⊢-lemma ρ)))
                                                                   (H.sym $ ·-/⊢ t₁ t₂ ρ) ⟩
  ...
  /⊢t-cong :
    ∀ {Γ₁ Δ₁ σ₁} {σ′₁ : Γ₁ ⊢ σ₁ type} {ρ̂₁ : Γ₁ ⇨̂ Δ₁} {ρ₁ : Sub T ρ̂₁}
      {Γ₂ Δ₂ σ₂} {σ′₂ : Γ₂ ⊢ σ₂ type} {ρ̂₂ : Γ₂ ⇨̂ Δ₂} {ρ₂ : Sub T ρ̂₂} →
    Γ₁ ≡ Γ₂ → Δ₁ ≡ Δ₂ → σ₁ ≅ σ₂ → σ′₁ ≅ σ′₂ → ρ̂₁ ≅ ρ̂₂ → ρ₁ ≅ ρ₂ →
    σ′₁ /⊢t ρ₁ ≅ σ′₂ /⊢t ρ₂
  /⊢t-cong P.refl P.refl H.refl H.refl H.refl H.refl = H.refl

  /⊢t⋆-cong :
    ∀ {Γ₁ Δ₁ σ₁} {σ′₁ : Γ₁ ⊢ σ₁ type} {ρ̂₁ : Γ₁ ⇨̂ Δ₁} {ρs₁ : Subs T ρ̂₁}
      {Γ₂ Δ₂ σ₂} {σ′₂ : Γ₂ ⊢ σ₂ type} {ρ̂₂ : Γ₂ ⇨̂ Δ₂} {ρs₂ : Subs T ρ̂₂} →
    Γ₁ ≡ Γ₂ → Δ₁ ≡ Δ₂ → σ₁ ≅ σ₂ → σ′₁ ≅ σ′₂ → ρ̂₁ ≅ ρ̂₂ → ρs₁ ≅ ρs₂ →
    σ′₁ /⊢t⋆ ρs₁ ≅ σ′₂ /⊢t⋆ ρs₂
  /⊢t⋆-cong P.refl P.refl H.refl H.refl H.refl H.refl = H.refl
...
module Unfolding-lemmas
  ...
  abstract
    ...
    ⋆-/⊢t⋆ : ∀ {Γ Δ} {ρ̂ : Γ ⇨̂ Δ} (ρs : Subs T ρ̂) → ⋆ /⊢t⋆ ρs ≡ ⋆
    ⋆-/⊢t⋆ ε        = P.refl
    ⋆-/⊢t⋆ (ρs ▻ ρ) = begin
      ⋆ /⊢t⋆ ρs /⊢t ρ  ≅⟨ /⊢t-cong P.refl P.refl H.refl (H.≡-to-≅ $ ⋆-/⊢t⋆ ρs) H.refl (H.refl {x = ρ}) ⟩
      ⋆ /⊢t ρ          ≡⟨ P.refl ⟩
      ⋆                ∎
      where open P.≡-Reasoning

    el-/⊢t : ∀ {Γ Δ} {ρ̂ : Γ ⇨̂ Δ} (t : Γ ⊢ k ⋆) (ρ : Sub T ρ̂) →
             el t /⊢t ρ ≅ _⊢_type.el (t /⊢ ρ)
    el-/⊢t t ρ =
      H.≡-subst-removable
        (λ v → _ ⊢ k U.el ˢ v type)
        (P.sym $ t /⊢-lemma ρ)
        _

    el-/⊢t⋆ : ∀ {Γ Δ} {ρ̂ : Γ ⇨̂ Δ} (t : Γ ⊢ k ⋆) (ρs : Subs T ρ̂) →
              el t /⊢t⋆ ρs ≅ _⊢_type.el (t /⊢⋆ ρs)
    el-/⊢t⋆ t ε        = H.refl
    el-/⊢t⋆ t (ρs ▻ ρ) = begin
      el t /⊢t⋆ ρs /⊢t ρ   ≅⟨ /⊢t-cong P.refl P.refl
                                (k-el-ˢ-cong P.refl (H.≡-to-≅ $ corresponds (app⋆ ρs) t))
                                (el-/⊢t⋆ t ρs) H.refl (H.refl {x = ρ}) ⟩
    ...
    π-/⊢t⋆ : ∀ {Γ Δ σ τ} {ρ̂ : Γ ⇨̂ Δ}
             ...
             π σ′ τ′ /⊢t⋆ ρs ≡ π (σ′ /⊢t⋆ ρs) (τ′ /⊢t⋆ ρs ↑⋆)
    π-/⊢t⋆ σ′ τ′ ε        = P.refl
    π-/⊢t⋆ σ′ τ′ (ρs ▻ ρ) = begin
      π σ′ τ′ /⊢t⋆ ρs /⊢t ρ                         ≅⟨ /⊢t-cong P.refl P.refl H.refl
                                                         (H.≡-to-≅ $ π-/⊢t⋆ σ′ τ′ ρs) H.refl (H.refl {x = ρ}) ⟩
    ...
    ƛ-/⊢⋆ : ∀ {Γ Δ σ} τ {ρ̂ : Γ ⇨̂ Δ}
            ...
            ƛ {τ = τ} σ′ t /⊢⋆ ρs ≡ ƛ (σ′ /⊢t⋆ ρs) (t /⊢⋆ ρs ↑⋆)
    ƛ-/⊢⋆ τ σ′ t ε        = P.refl
    ƛ-/⊢⋆ τ σ′ t (ρs ▻ ρ) = begin
      ƛ {τ = τ} σ′ t /⊢⋆ ρs /⊢ ρ                                 ≅⟨ /⊢-cong P.refl P.refl H.refl
                                                                      (H.≡-to-≅ $ ƛ-/⊢⋆ τ σ′ t ρs) H.refl (H.refl {x = ρ}) ⟩
    ...
    ·-/⊢⋆         t₁ t₂ ε        = H.refl
    ·-/⊢⋆ {τ = τ} t₁ t₂ (ρs ▻ ρ) = begin
        t₁ · t₂ /⊢⋆ ρs /⊢ ρ                    ≅⟨ /⊢-cong P.refl P.refl
                                                    (/̂-cong P.refl P.refl (H.refl {x = uc τ})
                                                       (▻̂-cong P.refl P.refl H.refl (H.refl {x = ⟦ ρs ⟧⇨⋆})
                                                          (H.≡-to-≅ $ corresponds (app⋆ ρs) t₂)))
                                                    (·-/⊢⋆ t₁ t₂ ρs) H.refl H.refl ⟩
  ...
  abstract
    ...
    mutual
      ...
      var-/⊢⋆-↑⁺⋆-⇒-/⊢t⋆-↑⁺⋆ hyp Γ⁺ ⋆ = begin
        ...
        ⋆ /⊢t⋆₂ ρs₂ ↑⁺⋆₂ Γ⁺  ∎
        where open P.≡-Reasoning
      var-/⊢⋆-↑⁺⋆-⇒-/⊢t⋆-↑⁺⋆ hyp Γ⁺ (el t) = H.≅-to-≡ (begin
        el t /⊢t⋆₁ ρs₁ ↑⁺⋆₁ Γ⁺   ≅⟨ Unfolding-lemmas.el-/⊢t⋆ T₁↦T t (ρs₁ ↑⁺⋆₁ Γ⁺) ⟩
        el (t /⊢⋆₁ ρs₁ ↑⁺⋆₁ Γ⁺)  ≅⟨ el-cong P.refl (H.≡-to-≅ $ var-/⊢⋆-↑⁺⋆-⇒-/⊢⋆-↑⁺⋆ hyp Γ⁺ t) ⟩
        el (t /⊢⋆₂ ρs₂ ↑⁺⋆₂ Γ⁺)  ≅⟨ H.sym $ Unfolding-lemmas.el-/⊢t⋆ T₂↦T t (ρs₂ ↑⁺⋆₂ Γ⁺) ⟩
        el t /⊢t⋆₂ ρs₂ ↑⁺⋆₂ Γ⁺   ∎)
        where open H.≅-Reasoning
      var-/⊢⋆-↑⁺⋆-⇒-/⊢t⋆-↑⁺⋆ hyp Γ⁺ (π σ′ τ′) = begin
        π σ′ τ′ /⊢t⋆₁ ρs₁ ↑⁺⋆₁ Γ⁺                                       ≡⟨ Unfolding-lemmas.π-/⊢t⋆ T₁↦T σ′ τ′ (ρs₁ ↑⁺⋆₁ Γ⁺) ⟩
        π (σ′ /⊢t⋆₁ ρs₁ ↑⁺⋆₁ Γ⁺) (τ′ /⊢t⋆₁ ρs₁ ↑⁺⋆₁ (Γ⁺ ▻ ⟦ σ′ ⟧type))  ≅⟨ π-cong P.refl H.refl H.refl
                                                                             (H.≡-to-≅ $ var-/⊢⋆-↑⁺⋆-⇒-/⊢t⋆-↑⁺⋆ hyp Γ⁺ σ′)
                                                                             (H.≡-to-≅ $ var-/⊢⋆-↑⁺⋆-⇒-/⊢t⋆-↑⁺⋆
                                                                                           hyp (Γ⁺ ▻ ⟦ σ′ ⟧type) τ′) ⟩
      ...
      var-/⊢⋆-↑⁺⋆-⇒-/⊢⋆-↑⁺⋆ hyp Γ⁺ (ƛ {τ = τ} σ′ t) = begin
        ...
        ƛ (σ′ /⊢t⋆₁ ρs₁ ↑⁺⋆₁ Γ⁺) (t /⊢⋆₁ ρs₁ ↑⁺⋆₁ (Γ⁺ ▻ ⟦ σ′ ⟧type))  ≅⟨ ƛ-cong P.refl H.refl H.refl
                                                                           (H.≡-to-≅ $ var-/⊢⋆-↑⁺⋆-⇒-/⊢t⋆-↑⁺⋆ hyp Γ⁺ σ′)
                                                                           (H.≡-to-≅ $ var-/⊢⋆-↑⁺⋆-⇒-/⊢⋆-↑⁺⋆ hyp (Γ⁺ ▻ ⟦ σ′ ⟧type) t) ⟩
        ƛ (σ′ /⊢t⋆₂ ρs₂ ↑⁺⋆₂ Γ⁺) (t /⊢⋆₂ ρs₂ ↑⁺⋆₂ (Γ⁺ ▻ ⟦ σ′ ⟧type))  ≡⟨ P.sym $ Unfolding-lemmas.ƛ-/⊢⋆ T₂↦T τ σ′ t (ρs₂ ↑⁺⋆₂ Γ⁺) ⟩
        ƛ {τ = τ} σ′ t /⊢⋆₂ ρs₂ ↑⁺⋆₂ Γ⁺                               ∎
        where open P.≡-Reasoning
      var-/⊢⋆-↑⁺⋆-⇒-/⊢⋆-↑⁺⋆ hyp Γ⁺ (t₁ · t₂) = H.≅-to-≡ (begin
        t₁ · t₂ /⊢⋆₁ ρs₁ ↑⁺⋆₁ Γ⁺                       ≅⟨ Unfolding-lemmas.·-/⊢⋆ T₁↦T t₁ t₂ (ρs₁ ↑⁺⋆₁ Γ⁺) ⟩
        (t₁ /⊢⋆₁ ρs₁ ↑⁺⋆₁ Γ⁺) · (t₂ /⊢⋆₁ ρs₁ ↑⁺⋆₁ Γ⁺)  ≅⟨ ·-cong P.refl H.refl H.refl
                                                            (H.≡-to-≅ $ var-/⊢⋆-↑⁺⋆-⇒-/⊢⋆-↑⁺⋆ hyp Γ⁺ t₁)
                                                            (H.≡-to-≅ $ var-/⊢⋆-↑⁺⋆-⇒-/⊢⋆-↑⁺⋆ hyp Γ⁺ t₂) ⟩
        (t₁ /⊢⋆₂ ρs₂ ↑⁺⋆₂ Γ⁺) · (t₂ /⊢⋆₂ ρs₂ ↑⁺⋆₂ Γ⁺)  ≅⟨ H.sym $ Unfolding-lemmas.·-/⊢⋆ T₂↦T t₁ t₂ (ρs₂ ↑⁺⋆₂ Γ⁺) ⟩
        t₁ · t₂ /⊢⋆₂ ρs₂ ↑⁺⋆₂ Γ⁺                       ∎)
        where open H.≅-Reasoning
